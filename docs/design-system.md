...
`@communicationos/ui` est la source unique des fondations visuelles et des composants partagés. Une valeur visuelle récurrente ne doit pas être redéfinie dans une feature.

## Fondations

- `tokens/index.ts` fournit les tokens typés utilisables en TypeScript et Framer Motion.
- `styles/index.css` expose les mêmes fondations sous forme de variables CSS `--cos-*`.
...
Le frontend importe `@communicationos/ui/styles.css`, puis les composants depuis `@communicationos/ui`. Les composants restent sans logique métier et acceptent les attributs HTML natifs.
